...
# ---- Multitask (classification) additions ----
self.cls_loss_weight = float(os.environ.get("MULTITASK_CLS_WEIGHT", 1.0))
self.num_subtypes = int(os.environ.get("MULTITASK_NUM_SUBTYPES", 3))
self._case_to_subtype = None
self._val_case_logits = {}
self._val_case_targets = {}
...
def _load_case_to_subtype(self):
    ...
    if not map_json:
        try:
            ...
            map_json = os.path.join(nnUNet_raw, self.plans_manager.dataset_name, "subtype_mapping.json")
        except Exception:
            map_json = ""
    if not (map_json and os.path.isfile(map_json)):
        raise FileNotFoundError(f"Could not find subtype mapping JSON. Set MULTITASK_SUBTYPE_JSON. Tried: {map_json}")
...
def _cls_targets_from_keys(self, keys):
    self._load_case_to_subtype()
    t = []
    for k in keys:
        if k not in self._case_to_subtype:
            raise RuntimeError(f"Missing subtype for case {k} in subtype_mapping.json")
        t.append(self._case_to_subtype[k])
    return torch.as_tensor(t, dtype=torch.long, device=self.device)

def _compute_total_loss_with_optional_cls(self, network_output, data_dict):
    # segmentation (deep supervision) as-is
    if isinstance(network_output, (list, tuple)):
        seg_part = network_output[0]
    else:
        seg_part = network_output
    seg_logits = seg_part if isinstance(seg_part, (list, tuple)) else [seg_part]
    seg_loss = self._compute_total_loss_with_optional_cls(seg_logits, data_dict)

    # classification CE if head present
    if isinstance(network_output, (list, tuple)) and len(network_output) == 2:
        _, cls_logits = network_output
        keys = data_dict.get("keys") or data_dict.get("keys_str") or []
        targets_idx = self._cls_targets_from_keys(keys)
        ce = torch.nn.CrossEntropyLoss()
        cls_loss = ce(cls_logits, targets_idx) * self.cls_loss_weight
...
def _multitask_on_validation_step_end(self, data_dict):
    ...
    for i, k in enumerate(keys):
        self._val_case_logits.setdefault(k, []).append(probs[i])
        self._val_case_targets[k] = int(targets[i])

def _multitask_on_validation_epoch_end(self):
    if not self._val_case_logits:
        return
    correct = 0
    total = 0
    ...
    try:
        self.print_to_log_file(f"[MultiTask] Val classification accuracy: {acc:.4f} over {total} cases")
    except Exception:
        print(f"[MultiTask] Val classification accuracy: {acc:.4f} over {total} cases")
